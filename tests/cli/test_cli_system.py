"""CLI tests for `bioamla system` commands."""
...
def test_system_group_help(runner: CliRunner) -> None:
...
def test_system_version(runner: CliRunner) -> None:
    data = VersionData(
...
def test_system_version_no_torch(runner: CliRunner) -> None:
    data = VersionData(
        bioamla_version="0.2.0",
        python_version="3.11.0 (main)",
        platform="Linux",
...
def test_system_version_error(runner: CliRunner) -> None:
...
def test_system_devices_cuda(runner: CliRunner) -> None:
    data = DevicesData(
        devices=[
    ...
    with patch("bioamla.system.util.get_device_info", return_value=data):
        result = runner.invoke(cli, ["system", "devices"])
    assert result.exit_code == 0
    assert "CUDA available" in result.output


def test_system_devices_mps(runner: CliRunner) -> None:
    data = DevicesData(
...
def test_system_devices_cpu(runner: CliRunner) -> None:
    data = DevicesData(
...
def test_system_devices_error(runner: CliRunner) -> None:
...
def test_system_deps_all_installed(runner: CliRunner) -> None:
    with patch("bioamla.system.dependency.check_all", return_value=_report(True)):
        result = runner.invoke(cli, ["system", "deps"])
    assert result.exit_code == 0
    assert "All system dependencies are installed" in result.output


def test_system_deps_missing_list(runner: CliRunner) -> None:
...
def test_system_deps_install(runner: CliRunner) -> None:
    with (
        patch(
        ...
        result = runner.invoke(cli, ["system", "deps", "--install", "-y"])
    assert result.exit_code == 0
    assert "Installing" in result.output


def test_system_deps_install_abort(runner: CliRunner) -> None:
    with patch("bioamla.system.dependency.check_all", return_value=_report(False)):
        result = runner.invoke(cli, ["system", "deps", "--install"], input="n\n")
    assert result.exit_code == 0
    assert "Aborted" in result.output


def test_system_deps_install_fails(runner: CliRunner) -> None:
    with (
...
def test_system_deps_check_error(runner: CliRunner) -> None:
