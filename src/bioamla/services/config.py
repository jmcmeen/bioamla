...
class ConfigService(BaseService):
    ...
    def is_in_project(self) -> ServiceResult[bool]:
        """
        Check if currently in a bioamla project.

        Returns:
            ServiceResult containing True if in a project
        """
        try:
            from bioamla.core.config import is_in_project as core_is_in_project

            result = core_is_in_project()

            return ServiceResult.ok(
                data=result,
                message="In project" if result else "Not in project",
            )
        except Exception as e:
            return ServiceResult.fail(f"Failed to check project status: {e}")

    def get_project_root(self) -> ServiceResult[Optional[str]]:
        """
        Get the root directory of the current bioamla project.

        Returns:
            ServiceResult containing the project root path or None
        """
        try:
            from bioamla.core.config import get_project_root as core_get_root

            result = core_get_root()

            if result:
                return ServiceResult.ok(
                    data=str(result),
                    message=f"Project root: {result}",
                )
            else:
                return ServiceResult.ok(
                    data=None,
            ...
            return ServiceResult.fail(f"Failed to get project root: {e}")
