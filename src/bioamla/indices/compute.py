...
def compute_all_indices(
    ...
    h_spectral = (
        spectral_entropy(audio, sample_rate, n_fft, hop_length) if include_entropy else None
    )
    h_temporal = (
        temporal_entropy(audio, sample_rate, n_fft, hop_length) if include_entropy else None
    )
...
# ==============================================================================
# Single-index dispatch + metadata (folded from the former IndicesService)
# ==============================================================================
...
def compute_index(
    ...
    **kwargs: Any,
    ...
    Raises:
        InvalidInputError: if ``index_name`` is not a known index.
    """
    ...
    if name == "ndsi":
        value, _, _ = compute_ndsi(audio, sample_rate, n_fft=1024, hop_length=hop_length)
        return value
    if name == "h_spectral":
        return spectral_entropy(audio, sample_rate, n_fft, hop_length)
    if name == "h_temporal":
        return temporal_entropy(audio, sample_rate, n_fft, hop_length)
    raise InvalidInputError(f"Unknown index: {index_name}. Available: {AVAILABLE_INDICES}")
