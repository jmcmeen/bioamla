...
This module folds the former ``core/detectors.py`` together with
``services/detection.py`` into plain, exception-raising functions and classes.
Functions return plain data and raise :class:`bioamla.exceptions.BioamlaError`
subclasses on failure (no ServiceResult wrappers, no repository DI).
...
from bioamla.audio.processing import bandpass_filter
...
def _load_audio_file(filepath: str | Path) -> tuple[np.ndarray, int]:
    ...
    try:
        ...
        raise AudioLoadError(f"Failed to load audio file {filepath}: {e}") from e
...
class BandLimitedEnergyDetector:
    ...
    Raises:
        InvalidDetectionParams: If ``high_freq <= low_freq`` or a frequency
            bound is negative.
    ...
    def detect(
        ...
        """
        try:
            return self._detect(audio, sample_rate, hop_length)
        except Exception as e:
            raise DetectionError(f"Energy detection failed: {e}") from e
    ...
    def detect_from_file(
        ...
        **kwargs,
        ...
        Raises:
            ...
            DetectionError: If detection fails.
        """
        audio, sample_rate = _load_audio_file(filepath)
...
class RibbitDetector:
    ...
    def detect(
        ...
        """
        try:
            return self._detect(audio, sample_rate)
        except Exception as e:
...
class CWTPeakDetector:
    ...
    Raises:
        InvalidDetectionParams: If both frequency bounds are given and
            ``high_freq <= low_freq``.
    ...
    def detect(
        ...
        try:
            ...
            raise DetectionError(f"Peak detection failed: {e}") from e
...
class AcceleratingPatternDetector:
    ...
    def detect(
        ...
        try:
            ...
            raise DetectionError(f"Accelerating pattern detection failed: {e}") from e
...
def _validate_freq_band(low_freq: float, high_freq: float) -> None:
    """Validate a frequency band, raising InvalidDetectionParams on bad input."""
    if low_freq < 0 or high_freq < 0:
        raise InvalidDetectionParams("Frequency bounds must be non-negative")
    if high_freq <= low_freq:
        raise InvalidDetectionParams(
            f"high_freq ({high_freq}) must be greater than low_freq ({low_freq})"
        )
...
def detect_all(
    ...
    Raises:
        DetectionError: If any detector fails.
    """
    all_detections = []
...
def export_detections(
    ...
    Raises:
        DetectionError: If writing the output file fails.
    """
    import csv
    import json
    ...
    try:
        ...
        if format == "json":
            output_path.write_text(
                json.dumps([d.to_dict() for d in detections], indent=2),
                encoding="utf-8",
            )
        else:
            if detections:
        ...
        raise DetectionError(f"Failed to export detections: {e}") from e
...
def batch_detect(
    ...
    Per-file failures are logged and recorded as an empty detection list so a
    ...
    for i, filepath in enumerate(filepaths, 1):
        ...
        try:
            if hasattr(detector, "detect_from_file"):
                ...
                audio, sr = _load_audio_file(filepath)
        ...
        except (AudioLoadError, DetectionError) as e:
